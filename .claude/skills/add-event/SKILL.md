---
description: Add a new sourced MQ-9 loss/addition event to src/data/inventory.json. Use when adding a documented shootdown, crash, or fleet addition with a source.
argument-hint: [one-line description, or paste a headline + source URL]
disable-model-invocation: true
allowed-tools: Read Edit Write AskUserQuestion Bash(npm run build) Bash(npm run typecheck)
---

Add a new event to the `events` array in `src/data/inventory.json` for:

$ARGUMENTS

Here is the current data so you can see existing ids/structure and pick the right insertion spot:
...
6. Summarize what you added (id, date, count, cause, sources) and remind me to
   review and commit. Do not commit unless I explicitly ask.
